...
func New(hashID hash.Identifier) *Ristretto {
	return &Ristretto{h2r.New(hashID)}
}
...
func (r *Ristretto) HashToGroup(input, dst []byte) group.Element {
	uniform := r.h2r.Expand(input, dst, ristrettoInputLength)
...
func (r *Ristretto) HashToScalar(input, dst []byte) group.Scalar {
	hashed := r.h2r.Expand(input, dst, ristrettoInputLength)
