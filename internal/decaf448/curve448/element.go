...
func (e *Element) affineAdd(element *Element) internal.Element {
	...
	case element.IsIdentity():
		return e
	case e.isEqual(element.copy().negate()) == 1:
		return e.Identity()
	case e.isEqual(element) == 1:
		return e.affineDouble()
	}

	var t0, t1, ll, x, y big.Int

	fp.Sub(&t0, &element.y, &e.y)
	fp.Sub(&t1, &element.x, &e.x)
	fp.Inv(&t1, &t1)
	fp.Mul(&ll, &t0, &t1)
...
func (e *Element) affineDouble() internal.Element {
	...
	var t0, t1, ll, two, three, x, y big.Int

	fp.Add(&two, fp.One(), fp.One())
	fp.Add(&three, &two, fp.One())

	fp.Mul(&t0, &three, &e.x)  // 3x
	fp.Mul(&t1, &two, a)       // 2A
	fp.Add(&t0, &t0, &t1)      // 3x+2A
	fp.Mul(&t0, &t0, &e.x)     // (3x+2A)x
	fp.Add(&t1, &t0, fp.One()) // (3x+2A)x+1
	fp.Mul(&t0, &two, b)       // 2B
	fp.Mul(&t0, &t0, &e.y)     // 2By
	fp.Inv(&t0, &t0)           // 1/2By
	fp.Mul(&ll, &t1, &t0)      // l = (3x^2+2Ax+1)/(2By)

	fp.Square(&t0, &ll)    // l^2
	fp.Mul(&t0, &t0, b)    // Bl^2
	fp.Sub(&t0, &t0, a)    // Bl^2-A
	fp.Sub(&t0, &t0, &e.x) // Bl^2-A-x
	fp.Sub(&x, &t0, &e.x)  // x' = Bl^2-A-2x

	fp.Sub(&t0, &e.x, &x) // x-x'
	fp.Mul(&t0, &t0, &ll) // l(x-x')
	fp.Sub(&y, &t0, &e.y) // y3 = l(x-x')-y1
...
func TestY() (*big.Int, *big.Int) {
	var x, y big.Int
	x.Set(baseX)
	polyToy2(&y, &x)
	if !fp.IsSquare(&y) {
		panic(errParamInvalidPointEncoding)
	}
	fp.SquareRoot(&y, &y)
	return &x, &y
}

// applies y2 = x(x2 + Ax + 1)
func polyToy2(y, x *big.Int) {
	fp.Add(y, x, a)
	fp.Mul(y, y, x)
	fp.Add(y, y, fp.One())
	fp.Mul(y, y, x)
